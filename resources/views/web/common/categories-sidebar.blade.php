@foreach($categories as $category)
  ...
  <div class="py-5 font-medium border-b border-gray-200 text-gray-600 tracking-wide {{ $category->order ? 'order-'.$category->order : 'order-last' }}">
    <div class="flex justify-between items-center w-full">
      ...
      <a href="/{{ $category->slug }}" class="ml-3 w-full font-medium text-gray-800 hover:text-primary-lighter block {{ $catSlug === $category->slug ? 'text-primary-lighter font-medium' : '' }}">
      {{ $category->name }}
      </a>
    </div>
      ...
      <div class="sub-categories pl-3 pt-5 overflow-hidden">
        @foreach($category->children as $subcat)
          ...
          <div class="flex items-center justify-between text-gray-500">
            <a href="/{{ $subcat->slug }}" class="w-full block py-2 text-gray-500 hover:text-primary-lighter {{ $subcatClass }}" data-cat-slug="">{{ $subcat->name }}</a>
          ...
          <div class="max-h-0 sub-categories overflow-hidden transition-half {{ $isOpen ? 'max-h-96' : '' }}" data-subcategory="{{ $subcat->slug }}">
            <div class="pl-4 my-5">
              @foreach($subcat->children as $subsubcat)
                <a href="/{{ $subsubcat->slug }}" class="w-full block py-2 text-gray-500 hover:text-primary-lighter {{ $catSlug === $subsubcat->slug ? 'text-primary-lighter font-medium' : '' }}">{{ $subsubcat->name }}</a>
              @endforeach
            </div>
          </div>
        @endforeach
