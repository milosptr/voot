    ...
    <div class="container mx-auto">
        <div class="relative flex items-center justify-between h-16">
            <div class="flex items-center px-2 lg:px-0 h-full">
                ...
                <div id="main-menu" class="lg:block lg:ml-6">
                    <div class="flex flex-col sm:flex-row space-x-4 ">
                        ...
                        <a href="/um-okkur"
                            class="text-gray-600 hover:text-primary-light px-3 py-2 rounded-md text-xl small-caps {{ $route === 'about' ? 'font-semibold' : '' }}">{{ __('header.about') }}</a>
                        <a href="/thjonusta"
                            class="text-gray-600 hover:text-primary-light px-3 py-2 rounded-md text-xl small-caps {{ $route === 'services' ? 'font-semibold' : '' }}">{{ __('header.services') }}</a>
                        <a href="/vorumerkin-okkar"
                            class="text-gray-600 hover:text-primary-light px-3 py-2 rounded-md text-xl small-caps {{ $route === 'brands' ? 'font-semibold' : '' }}">{{ __('header.our_brands') }}</a>
                        <a href="/vorur"
                            class="text-gray-600 hover:text-primary-light px-3 py-2 rounded-md text-xl small-caps {{ $route === 'all_products' ? 'font-semibold' : '' }}">{{ __('header.products') }}</a>
                        <a href="/hafa-samband"
                            class="text-gray-600 hover:text-primary-light px-3 py-2 rounded-md text-xl small-caps {{ $route === 'contact' ? 'font-semibold' : '' }}">{{ __('header.contact') }}</a>
                    </div>
                </div>
            </div>
                    ...
                    <div class="relative z-0">
                        ...
                        <form action="/products/search" method="GET" class="sm:mr-16">
                            <input id="search" name="q"
                                ...
                                placeholder="{{ __('header.search') }}" type="search">
                        </form>
                    </div>
                </div>
            </div>
          ...
          <img src="/images/{{ __('header.language') }}.png" width="24" alt="en" />
                ...
                @endif
                @if (Cookie::get('auth_id') && !auth()->user())
                    <a href="/cart-request" class="ml-4 text-gray-600 hover:text-primary-light relative"
                        aria-label="Cart">
                        <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round" class="feather feather-shopping-cart">
                        ...
                        @php $inCart = App\Models\RequestPriceCart::cartNumber(Cookie::get('auth_id')); @endphp
                        @if ($inCart)
                            <div
                                class="absolute right-0 top-0 items-center justify-center text-center w-4 h-4 text-xs font-medium -mt-2 -mr-2 bg-red-500 text-white rounded-full">
                                {{ $inCart }}
                ...
                @endif
                <a href="{{ auth()->user() != null ? '/backend' : route('registration-page') }}"
