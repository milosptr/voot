<template>
  <div>
    <div class="flex gap-10 mt-8">
      <Select v-if="variations.length" label="Select option" :options="variations" :color="true" classes="min-w-150" :firstDefault="true" @selected="selectVariant($event)" />
      <Select :label="quantityTranslated" :options="qtyOptions" :firstDefault="true" @selected="qty = $event.value" />
      <div class="w-20 text-center cursor-pointer">
    ...
    <div class="flex items-center gap-2">
      <button type="submit" class="mt-8 w-1/2 text-center text-white border border-primary-lighter bg-primary-lighter px-6 py-2 font-medium rounded-md hover:bg-primary-light cursor-pointer shadow-sm" @click="addToCart">
        {{ addToCartLabel }}
      </button>
    </div>
  </div>
