<template>
  <div class="">
    <div class="mb-4">
        ...
        <input v-model="newsletter.title" type="text" id="title" class="mt-1 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"  />
    </div>
    <div class="mb-4">
      <label for="content" class="block text-sm font-medium text-gray-700 mt-6 mb-1">Newsletter content</label>
   ...
   <div class="flex justify-between">
    ...
    <div tabindex="0" class="w-32 select-none text-center text-red-500 border border-red-500 px-6 py-2 mt-5 text-sm font-normal ml-auto rounded-md hover:bg-red-500 hover:text-white cursor-pointer" @keydown.enter="deleteNewsletter" @click="deleteNewsletter">
      Delete
    </div>
    <div tabindex="0" class="w-32 select-none text-center text-white border border-primary-light bg-primary-light px-6 py-2 mt-5 ml-4 text-sm font-normal rounded-md hover:bg-primary-lighter hover:text-white cursor-pointer" @keydown.enter="save" @click="save">
      Save
    </div>
   </div>
  ...
  export default {
    ...
    mounted() {
      axios.get('/api/newsletter')
        .then((res) => {
          this.newsletter.id = res.data.data.id
          this.newsletter.title = res.data.data.title
          this.newsletter.content = res.data.data.content
          this.addedProducts = res.data.data.products
        })
    },
    methods: {
      openModal() {
      ...
      save() {
        this.newsletter.products = JSON.stringify(this.addedProducts.map((p) => p.id))
        axios.post('/api/newsletter', this.newsletter)
          .then((res) => {
            this.$emit('saved')
          })
      },
      deleteNewsletter() {
        axios.delete('/api/newsletter/' + this.newsletter.id)
        location.reload()
      }
    }
  }
